...
Animation::Animation(DirectXGame* dx, int spritePerColumn, int spritePerRow, int fps, char* fileName, D3DCOLOR transparentColor, Direction defautDirection) :
DXTexture(dx)
{
...
void Animation::Draw(BaseObject* p, int index, Direction direction)
{
	...
	D3DXVECTOR2 pos;
	pos.x = p->_position.X - Camera::GetInstance()->_position.X;
	pos.y = p->_position.Y - Camera::GetInstance()->_position.Y;
	pos.x += (p->_width - r.Width)/2;
	pos.y += (p->_height - r.Height) / 2;
